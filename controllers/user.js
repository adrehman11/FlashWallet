  ...
  try {
    ...
    if (exist) {
      ...
      await User.update(
        {
          otpCode: otpCode,
          otpCode_timestamp: otpCode_timestamp,
          udid: req.body.udid,
        },
    ...
    if (req.body.referralCode) {
      ...
      let allReferralData = await ReferralHistory.findAll({
        where: { user1: user1.id },
        include: [{ model: User, as: "referee", attributes: ["udid"] }],
      });
      const isUdidPresent = allReferralData.some(
        (referral) => referral.dataValues.referee.udid === req.body.udid
      );
      if (user1) {
...
exports.createUserImportedWallet = async (req, res) => {
  try {
    let verifyRSVToken = await verifyEthSign(
...
exports.OtpCodeVerification = async (req, res) => {
  try {
    ...
    const OTP_EXP_MINUTES = 2;
    const user = await User.findOne({ where: { email, isMPC: true } });
    if (!user) {
      return res.status(404).json({ status: false, msg: "No user found." });
    }

    if (user.otpLockedUntil && moment().isBefore(moment(user.otpLockedUntil))) {
      const minsLeft = moment(user.otpLockedUntil).diff(moment(), "minutes");
      return res.status(429).json({
        status: false,
        msg: `Too many failed attempts. Try again in ${minsLeft} minute(s).`,
      });
    }

    if (!user.otpCode || !user.otpCode_timestamp) {
      return res.status(400).json({ msg: "Please request a new OTP." });
    }

    const now = moment();
    const sentAt = moment(parseInt(user.otpCode_timestamp, 10));
    const elapsedMinutes = now.diff(sentAt, "minutes");

    if (elapsedMinutes > OTP_EXP_MINUTES) {
      await user.update({
        otpCode: null,
        otpCode_timestamp: null,
        otpAttempts: 0,
        otpLockedUntil: null,
      });
      return res
        .status(400)
        .json({ msg: "Code expired. Please request a new code." });
    }
    ...
    if (otpCode !== user.otpCode) {
      ...
      if (newAttempts >= MAX_ATTEMPTS) {
        updateData.otpLockedUntil = moment()
          .add(LOCK_MINUTES, "minutes")
          .toDate();
      ...
      return res.status(409).json({
        status: false,
        msg: "Invalid code. Please request a new one.",
      });
    }
  ...
  try {
    ...
    if (user.otpCode_timestamp) {
      ...
      if (elapsedTime < 2) {
        return res
          .status(400)
          .json({ msg: "OTP email already sended to your email" });
      }
    }
    let otpCode = await otp_code();
    let otpCode_timestamp = Date.now();
...
exports.secret_phrase_1 = async (req, res) => {
  try {
    let user = req.user;
    let userRecord = await SecretPhrase1.findOne({
      where: { user_id: user.id },
    });
    if (userRecord) {
      return res.status(400).json({ msg: "User Secret Phrase already set" });
    }
    await SecretPhrase1.create({
      secret_phrase_1: req.body.secret_phrase_1,
      user_id: user.id,
    });
...
exports.setReferralCode = async (req, res) => {
  try {
    ...
    let codeAlreadyexsist = await User.findOne({
      where: { referral_code: req.body.code },
    });
    if (codeAlreadyexsist) {
      return res
        .status(401)
        .json({ msg: "This Referral code is  Already exsist" });
    }
    await User.update(
...
exports.getEarnedPoints = async (req, res) => {
  try {
    let user = req.user;
    let dataPoints = await ReferralHistory.findAll({
      where: { user1: user.id, isClaimed: false },
      attributes: [
        "id",
...
exports.ClaimEarnedPoints = async (req, res) => {
  try {
    let user = req.user;
    let data = await ReferralHistory.findOne({
      where: { id: req.body.id, isClaimed: false },
      attributes: [
        "id",
        "user1",
        [Sequelize.literal("referralPoints + NFTpoints"), "totalPoints"],
    ...
    await FlashPoints.create({
      user_id: data.user1,
      points: data.dataValues.totalPoints,
      description: `Earned ${data.dataValues.totalPoints} Points for referring a User`,
    });
  ...
  try {
    ...
    const response = await axios.post(
      ...
      },
      {
        ...
        params: {
          apiKey:
            "m8ciibQ3rhBrz6FoE7pXSEE3mwGx9e0FsNm16sULMfyGR_ZFCHyjs9F.UbZmKQFs",
        },
      }
    );
